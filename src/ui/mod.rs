...
const WARN: Color32 = Color32::from_rgb(240, 160, 30);
...
pub fn draw(app: &mut ChunkyApp, ctx: &egui::Context, frame: &mut eframe::Frame) {
    ...
    egui::TopBottomPanel::top("top_bar")
        .frame(Frame::none().fill(Color32::from_rgb(18, 18, 26)).inner_margin(Margin::symmetric(12.0, 8.0)))
        .show(ctx, |ui| {
            draw_top_bar(app, ui);
        });
    ...
    egui::TopBottomPanel::bottom("status_bar")
        .frame(Frame::none().fill(Color32::from_rgb(18, 18, 26)).inner_margin(Margin::symmetric(12.0, 4.0)))
        .show(ctx, |ui| {
...
fn draw_top_bar(app: &mut ChunkyApp, ui: &mut Ui) {
    ui.horizontal(|ui| {
        ...
        // Import
        let import_btn = Button::new(RichText::new("📂 Import").color(TEXT))
        ...
        let vox_btn = Button::new(RichText::new("⚡ Voxelize").color(if can_voxelize { ACCENT } else { TEXT_DIM }))
            .fill(if can_voxelize { ACCENT_DIM } else { Color32::from_rgb(42, 42, 58) });
        ...
        let schem_btn = Button::new(RichText::new("💾 .schem").color(TEXT))
            .fill(Color32::from_rgb(42, 42, 58));
        if ui.add_enabled(can_export, schem_btn).clicked() {
            app.export_schematic();
        }

        let mca_btn = Button::new(RichText::new("🗺 .mca").color(TEXT))
            .fill(Color32::from_rgb(42, 42, 58));
        ...
        for (label, mode) in [("Voxel", ViewMode::Voxel), ("Mesh", ViewMode::Mesh), ("Blocks", ViewMode::Blocks)] {
        ...
        ui.separator();
        ui.checkbox(&mut app.show_grid, RichText::new("Grid").color(TEXT_DIM).small());
        ui.checkbox(&mut app.show_chunk_boundaries, RichText::new("Chunks").color(TEXT_DIM).small());

        // Right-align: stats
        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
            if let Some(grid) = &app.voxel_grid {
                let voxels = grid.total_voxels();
                ui.label(RichText::new(format!("{} blocks", voxels)).color(ACCENT).small());
                ui.separator();
            }
            if let Some(scene) = &app.scene {
                ui.label(RichText::new(format!("{} tris", scene.total_triangles())).color(TEXT_DIM).small());
            }
        });
    });
}
...
fn draw_left_panel(app: &mut ChunkyApp, ui: &mut Ui) {
    ...
    Frame::none().inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
        if let Some(scene) = &app.scene {
            ...
            ui.label(RichText::new(format!("{} meshes · {} triangles",
                scene.meshes.len(), scene.total_triangles()))
                .color(TEXT_DIM).small());

            if let Some((min, max)) = scene.bounds() {
                let size = max - min;
                ui.label(RichText::new(format!("Bounds: {:.2} × {:.2} × {:.2} units",
            ...
            let drop_area = ui.add_sized(
                ...
                Button::new(RichText::new("📂  Drop .obj / .glb / .stl\n  or click to browse").color(TEXT_DIM))
    ...
    Frame::none().inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            ui.add(Slider::new(&mut app.transform_scale, 0.01..=100.0)
                .logarithmic(true)
                .suffix("×"));
        });
    ...
    Frame::none().inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ComboBox::from_id_source("vox_mode")
                ...
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut app.voxel_settings.mode, VoxelizeMode::Surface, "Surface");
                    ui.selectable_value(&mut app.voxel_settings.mode, VoxelizeMode::Solid, "Solid");
                    ui.selectable_value(&mut app.voxel_settings.mode, VoxelizeMode::Hybrid, "Hybrid");
                });
        });
        ...
        ui.horizontal(|ui| {
            ...
            ui.add(Slider::new(&mut app.voxel_settings.resolution, 4.0..=512.0)
                .logarithmic(true)
                .suffix(" b/u"));
        });

        if app.voxel_settings.mode == VoxelizeMode::Solid || app.voxel_settings.mode == VoxelizeMode::Hybrid {
            ui.horizontal(|ui| {
                ui.label(RichText::new("Infill").color(TEXT_DIM).small());
                ui.add(Slider::new(&mut app.voxel_settings.infill_density, 0.0..=1.0)
                    .fixed_decimals(2));
            });
        }
        ...
        if let Some(scene) = &app.scene {
            if let Some((_min, _max)) = scene.bounds() {
                ...
                let (val, unit) = if est > 1_000_000 { (est / 1_000_000, "M") }
                    else if est > 1_000 { (est / 1_000, "k") } else { (est, "") };
                ui.label(RichText::new(format!("~{}{} voxels (max)", val, unit))
                    .color(TEXT_DIM).small());
            }
        }
    });
    ...
    Frame::none().inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
        ui.checkbox(&mut app.optimize_settings.remove_hidden, "Remove hidden faces");
        ui.horizontal(|ui| {
            ui.checkbox(&mut app.optimize_settings.noise_filter, "Noise filter");
            if app.optimize_settings.noise_filter {
                ui.add(Slider::new(&mut app.optimize_settings.noise_threshold, 1..=6)
                    .text("min neighbors"));
            }
        });
    });
...
fn draw_right_panel(app: &mut ChunkyApp, ui: &mut Ui) {
    ...
    Frame::none().inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            ComboBox::from_id_source("mc_version")
                ...
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut app.palette_settings.version, MinecraftVersion::Java121, "Java 1.21");
                    ui.selectable_value(&mut app.palette_settings.version, MinecraftVersion::Java120, "Java 1.20");
                    ui.selectable_value(&mut app.palette_settings.version, MinecraftVersion::Java118, "Java 1.18");
                });
        });
        ...
        ui.horizontal(|ui| {
            ...
            ComboBox::from_id_source("pal_filter")
                ...
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut app.palette_settings.filter, PaletteFilter::All, "All blocks");
                    ui.selectable_value(&mut app.palette_settings.filter, PaletteFilter::ConcretesOnly, "Concrete only");
                    ui.selectable_value(&mut app.palette_settings.filter, PaletteFilter::WoolOnly, "Wool only");
                    ui.selectable_value(&mut app.palette_settings.filter, PaletteFilter::TerraCottaOnly, "Terracotta only");
                });
        });
    ...
    Frame::none().inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            ComboBox::from_id_source("exp_version")
                ...
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut app.export_settings.version, MinecraftVersion::Java121, "Java 1.21");
                    ui.selectable_value(&mut app.export_settings.version, MinecraftVersion::Java120, "Java 1.20");
                    ui.selectable_value(&mut app.export_settings.version, MinecraftVersion::Java118, "Java 1.18");
                });
        });
        ...
        ui.add_enabled_ui(can_export, |ui| {
            ui.vertical(|ui| {
                ...
                if schem.clicked() { app.export_schematic(); }
                ...
                if mca.clicked() { app.export_region_files(); }
            });
        });

        if !can_export {
            ui.label(RichText::new("Voxelize model first").color(TEXT_DIM).small());
        }
        ...
        if let Some(grid) = &app.voxel_grid {
            ...
            ui.label(RichText::new(format!("Total blocks: {}", total)).color(TEXT_DIM).small());
            ui.label(RichText::new(format!("Chunks: {} ({} × 32³)", chunks, chunks)).color(TEXT_DIM).small());

            if let Some((min, max)) = grid.bounds_voxel() {
                let size = max - min + IVec3::ONE;
                ui.label(RichText::new(format!("Dimensions: {}×{}×{} blocks", size.x, size.y, size.z))
                    .color(TEXT_DIM).small());
            }
        }
    });
    ...
    Frame::none().inner_margin(Margin::symmetric(12.0, 8.0)).show(ui, |ui| {
        ui.label(RichText::new("Orbit: Left drag\nZoom: Scroll\nPan: Right drag / Middle drag").color(TEXT_DIM).small());
    });
}

// ─── Viewport ─────────────────────────────────────────────────────────────────

fn draw_viewport(app: &mut ChunkyApp, ui: &mut Ui, frame: &mut eframe::Frame) {
    ...
    if drag_delta.length() > 0.0 {
        match app.drag_button {
            Some(PointerButton::Primary) => {
                app.camera.orbit(glam::Vec2::new(drag_delta.x, drag_delta.y));
            }
    ...
    if response.double_clicked() {
        if let Some(grid) = &app.voxel_grid {
            if let Some((min, max)) = grid.bounds_voxel() {
                let min_w = grid.voxel_to_world(min);
                let max_w = grid.voxel_to_world(max);
                app.camera.fit_to_bounds(min_w, max_w);
            }
    ...
    ui.painter().add(egui_wgpu::Callback::new_paint_callback(
        ...
        ViewportCallback {
            ...
            has_content: app.voxel_grid.is_some(),
        },
    ));
...
fn draw_status_bar(app: &mut ChunkyApp, ui: &mut Ui) {
    ui.horizontal(|ui| {
        if app.is_working {
            ...
            ui.painter().rect_filled(bar_rect, 2.0, Color32::from_rgb(40, 40, 55));
            ...
            let color = if app.status_message.starts_with("Error") { ERR }
                else if app.status_message.starts_with("Loaded") || app.status_message.contains("blocks") { ACCENT }
                else { TEXT_DIM };
...
fn section_header(ui: &mut Ui, text: &str) {
    ...
    ui.painter().rect_filled(rect, 0.0, Color32::from_rgb(26, 26, 38));
    ui.painter().text(
