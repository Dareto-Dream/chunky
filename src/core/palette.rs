...
pub fn apply_palette(grid: &mut VoxelGrid, db: &BlockDatabase, settings: &PaletteSettings) {
    let allowed = match settings.filter {
        ...
        PaletteFilter::Custom => {
            if settings.custom_ids.is_empty() {
                None
            } else {
                Some(settings.custom_ids.clone())
            }
        }
    };
